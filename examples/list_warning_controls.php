...
if (! $token) {
    ...
    echo "Usage: PATROWL_API_TOKEN=your_token PATROWL_DEFAULT_ORGANIZATION_ID=1 php examples/list_warning_controls.php\n";
    exit(1);
}
...
try {
    echo "Fetching controls with Potentially Impacted (Warning) result...\n";

    // Filter by result = 3 (PotentiallyImpacted)
    $controls = $client->controls()->all([
        'result' => ControlResultEnum::PotentiallyImpacted->value,
    ]);
    ...
    echo "----------------------------------------\n";
    echo "Found {$count} warning controls.\n";
    echo "Done.\n";
} catch (Exception $e) {
